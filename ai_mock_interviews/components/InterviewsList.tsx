...
export default function InterviewsList({
  ...
  const formatScoreDetail = (interview: Interview) => {
    const obtained = interview.scoreDetail?.obtained;
    const total = interview.scoreDetail?.total;
    if (obtained != null && total != null) {
      return `${obtained}/${total} (${interview.score}%)`;
    }
    if (total != null) {
      return `${interview.score}/${total}`;
    }
    return `${interview.score}%`;
  };

  const displayedInterviews = filteredInterviews.slice(0, displayCount);
  const hasMore = displayCount < filteredInterviews.length;
  ...
  return (
    <>
      ...
      {/* Load More Button */}
      {hasMore && filteredInterviews.length > 0 && (
        <div className="mt-6 text-center">
          ...
          >
            Load More Interviews ({filteredInterviews.length - displayCount} remaining)
          </Button>
        </div>
      )}
    </>
